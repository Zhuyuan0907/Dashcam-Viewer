...
test("queue limits per-user execution and records result only after finalization", async () => {
  const db = createDb(":memory:"),
    tasks = new BackgroundTasks(db, 2, 1),
    sse = new SSERegistry();
  let release!: () => void;
  const gate = new Promise<void>((r) => (release = r)),
    first = sse.create("a");
  let secondStarted = false;
  const a = tasks.enqueue(
  ...
  const second = sse.create("b");
  const b = tasks.enqueue(
    { type: "clip", owner: 1, target: "b", payload: {}, key: "b" },
    second,
    async () => {
      secondStarted = true;
      second.push({ stage: "done" });
      second.close();
    },
  );
  await until(() => tasks.get(a)?.status === "running");
  assert.equal(secondStarted, false);
  assert.equal(tasks.get(b)?.status, "queued");
  release();
...
test("restart marks persistent nonterminal work interrupted and keeps owner history isolated", () => {
  const db = createDb(":memory:");
  new BackgroundTasks(db);
  db.prepare(
    "INSERT INTO background_jobs VALUES ('a','trim',1,'t','{}','trim:t','running','encode',20,'',NULL,1,1)",
  ).run();
...
test("a runner that omits its terminal event must never report success", async () => {
  const db = createDb(":memory:"),
    tasks = new BackgroundTasks(db),
    sse = new SSERegistry();
  const channel = sse.create("incomplete");
  const id = tasks.enqueue(
    { type: "import", owner: 1, target: "batch", payload: {}, key: "incomplete" },
    channel,
    async () => {
      channel.close();
    },
  );
  await until(() => tasks.get(id)?.status === "failed");
  db.close();
});
