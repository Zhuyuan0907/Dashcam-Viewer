...
(() => {
  ...
  const normalize = (value) =>
    Object.hasOwn(palettes, value) ? value : value === "dark" ? "slate" : "harbor";
  const apply = (value) => {
    const name = normalize(value);
    document.documentElement.dataset.palette = name;
    document.documentElement.removeAttribute("data-theme");
    try {
      localStorage.setItem("dc.palette", name);
    } catch {}
  ...
  let saved = "harbor";
  try {
    saved = localStorage.getItem("dc.palette") || localStorage.getItem("dc.theme") || saved;
  } catch {}
  apply(saved);
})();
