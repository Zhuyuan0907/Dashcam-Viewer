...
function renderJobs() {
  ...
  for (const j of rows) {
    const card = node("article", undefined, "info-card");
    card.style.margin = "16px 0";
    card.append(
      node("h2", jobNames[j.type] || j.type),
      node(
        "p",
        (jobStates[j.status] || j.status) + " · " + new Date(j.created_at).toLocaleString(),
      ),
    );
    const progress = node("progress");
    progress.max = 100;
    progress.value = j.progress;
    progress.setAttribute("aria-label", jobStates[j.status] || j.status);
    card.append(progress, node("p", j.message));
    const actions = node("div", undefined, "form-row");
    for (const [action, label, allowed] of [
      ["cancel", "取消工作", j.can_cancel],
      ["retry", "重新執行", j.can_retry],
    ])
      if (allowed) {
        const button = node("button", label, "btn btn--ghost");
        button.onclick = async () => {
          button.disabled = true;
          try {
            await apiFetch(`/api/jobs/${j.id}/${action}`, { method: "POST" });
            await loadJobs();
          } catch (e) {
    ...
    if (j.result?.clip?.id) {
      const link = node("a", "查看片段", "btn");
      link.href = "/clips#clip-" + j.result.clip.id;
      actions.append(link);
    } else if (j.type !== "import") {
      const link = node("a", "回到旅程", "btn");
      link.href = "/trip/" + encodeURIComponent(j.target);
      actions.append(link);
    } else if (j.status === "succeeded" || j.status === "partial") {
      const link = node("a", "瀏覽旅程", "btn");
      link.href = "/browse";
      actions.append(link);
    }
    card.append(actions);
...
async function loadJobs() {
  ...
  try {
    ...
    jobRows = rows;
    renderJobs();
    document.getElementById("connection").textContent = "狀態已更新";
    document.getElementById("more").hidden = rows.length < jobLimit;
  } catch (e) {
    document.getElementById("connection").textContent =
      "連線暫時中斷，背景工作不受影響：" + e.message;
  } finally {
    jobLoading = false;
  }
}
