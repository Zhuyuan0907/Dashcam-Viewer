...
interface Replacement { target: string; staged: string; rollback: string }
async function rollback(entries: Replacement[]): Promise<void> {
  for (const e of entries) {
    if (!withinTrips(e.target) || !withinTrips(e.rollback)) throw new Error('Invalid recovery path');
...
async function clean(entries: Replacement[]): Promise<void> {
  for (const e of entries) for (const p of [e.rollback, e.staged]) {
    await fs.rm(p, { force: true }).catch(() => {});
  }
}
export async function recoverMediaCommits(db: DB): Promise<number> {
  const rows = db.prepare('SELECT id, entries FROM media_commits').all() as { id: string; entries: string }[];
...
export async function commitMedia(
  db: DB, tripId: string, files: { target: string; staged: string }[], commit: () => void,
): Promise<void> {
  const id = randomUUID();
  ...
  } catch (error) { await clean(entries); throw error; }
  try {
    db.prepare('INSERT INTO media_commits(id, trip_id, entries) VALUES (?,?,?)')
      .run(id, tripId, JSON.stringify(entries));
  } catch (error) { await clean(entries); throw error; }
  try {
