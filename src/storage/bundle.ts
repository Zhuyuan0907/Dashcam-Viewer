...
interface Manifest {version:1; source:string; created_at:string; files:{path:string;sha256:string}[]}
...
function checkDatabase(file:string):void {
  ...
  try {if(db.pragma('quick_check',{simple:true})!=='ok')throw Error('SQLite integrity check failed');}
  finally {db.close();}
}
...
export async function createBundle(source:string,destination:string):Promise<Manifest> {
  ...
  if(destination===source || destination.startsWith(source+path.sep))throw Error('備份必須放在資料目錄以外');
  ...
  const manifest:Manifest={version:1,source,created_at:new Date().toISOString(),files:[]};
  for(const file of files)manifest.files.push({path:file,sha256:await fileHash(path.join(destination,'data',file))});
...
export async function verifyBundle(directory:string):Promise<Manifest> {
  ...
  if(manifest.version!==1 || typeof manifest.source!=='string' || !Array.isArray(manifest.files))throw Error('無效備份清單');
  const data=path.join(directory,'data'),actual=await filesUnder(data);
  if(actual.length!==manifest.files.length)throw Error('備份檔案數不符');
  const remaining=new Set(actual);
  for(const entry of manifest.files) {
    if(typeof entry.path!=='string' || !isSafeRelative(entry.path) || !remaining.delete(entry.path))throw Error('不安全或重複的備份路徑');
    if(await fileHash(safeJoin(data,entry.path))!==entry.sha256)throw Error(`備份校驗失敗：${entry.path}`);
  }
...
export async function restoreBundle(directory:string,destination:string):Promise<void> {
  ...
  for (const name of await fs.readdir(path.join(directory,'data'))) {
    await fs.cp(path.join(directory,'data',name),path.join(destination,name),{recursive:true,errorOnExist:true,force:false});
  }
