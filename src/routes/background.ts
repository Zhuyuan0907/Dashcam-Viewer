...
export function registerBackground(app: FastifyInstance, ctx: AppContext): void {
  const preHandler = makeRequireUser(ctx),
    tasks = ctx.tasks!;
  const publicRow = (r: WorkRow) => ({
    id: r.id,
    type: r.type,
    target: r.target,
    status: r.status,
    ...
    can_cancel: tasks.canCancel(r.id),
    can_retry:
      tasks.retryable(r) &&
      (r.type !== "import" || ctx.sessions.get(r.target)?.status === "active"),
  });
  ...
  app.post<{ Params: { id: string } }>("/api/jobs/:id/cancel", { preHandler }, (req, reply) => {
    ...
    if (!row || row.owner_id !== req.user!.id)
      return reply.code(404).send({ detail: "工作不存在" });
    if (!tasks.cancel(row.id))
      return reply.code(409).send({ detail: "此階段無法取消，請等待結果儲存完成" });
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      if (["clip", "trim"].includes(row.type) && !JSON.parse(row.payload).expected_version)
        return reply.code(409).send({ detail: "舊工作缺少來源版本，請回旅程頁重新確認範圍" });
      const routes = {
        clip: "/api/trip-clips/",
        trim: "/api/trip-trim/",
        restore: "/api/trip-trim/",
        import: "/api/upload-sessions/",
      };
      const result = await app.inject({
        method: row.type === "restore" ? "DELETE" : "POST",
        url:
          routes[row.type] +
          encodeURIComponent(row.target) +
          (row.type === "import"
            ? "/confirm?gap_min=" + encodeURIComponent(JSON.parse(row.payload).gapMin ?? 15)
            : ""),
        headers: {
          cookie: req.headers.cookie ?? "",
          ...(row.type === "clip" || row.type === "trim"
            ? { "content-type": "application/json" }
            : {}),
        },
        ...(row.type === "clip" || row.type === "trim" ? { payload: JSON.parse(row.payload) } : {}),
      });
